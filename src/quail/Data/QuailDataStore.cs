...
public sealed class QuailDataStore(IConfiguration configuration)
{
    ...
    public async Task<List<User>> GetUsersByEmailsAsync(IReadOnlyCollection<string> emails, CancellationToken cancellationToken = default)
    {
        ...
        var rows = await connection.QueryAsync<UserRow>(new CommandDefinition(
            ...
            cancellationToken: cancellationToken));
        return rows.Select(ToUser).ToList();
    }

    public async Task InsertMailboxAsync(Mailbox mailbox, CancellationToken cancellationToken = default)
    {
        await using var connection = await this.OpenConnectionAsync(cancellationToken);
        var mailboxId = await connection.QuerySingleOrDefaultAsync<int?>(new CommandDefinition(
            """
    ...
    {
        await using var connection = await this.OpenConnectionAsync(cancellationToken);
        var row = await connection.QuerySingleOrDefaultAsync<MailboxRow>(new CommandDefinition(
            """
    ...
    {
        await using var connection = await this.OpenConnectionAsync(cancellationToken);
        var rows = await connection.QueryAsync<MailboxRow>(new CommandDefinition(
            """
            SELECT Id, Guid, UserId, Name, UidValidity, NextUid, SpecialUse
            FROM Mailboxes
            WHERE UserId = @UserId
    ...
    {
        await using var connection = await this.OpenConnectionAsync(cancellationToken);
        var rows = await connection.QueryAsync<MailboxStatsRow>(new CommandDefinition(
            """
            SELECT
                mb.Id,
                mb.Name,
    ...
    {
        await using var connection = await this.OpenConnectionAsync(cancellationToken);
        var uid = await connection.QuerySingleOrDefaultAsync<long?>(new CommandDefinition(
            """
            UPDATE Mailboxes
    ...
    {
        await using var connection = await this.OpenConnectionAsync(cancellationToken);
        var messageId = await connection.QuerySingleOrDefaultAsync<int?>(new CommandDefinition(
            """
    ...
    {
        await using var connection = await this.OpenConnectionAsync(cancellationToken);
        var mailboxMessageId = await connection.QuerySingleOrDefaultAsync<int?>(new CommandDefinition(
            """
    ...
    public async Task DeleteMailboxMessagesAsync(IReadOnlyCollection<int> mailboxMessageIds, CancellationToken cancellationToken = default)
    {
        ...
        await connection.ExecuteAsync(new CommandDefinition(
            """
            DELETE FROM MailboxMessages
            WHERE Id IN @Ids;
            """,
            new
            {
                Ids = mailboxMessageIds.ToArray()
            },
            cancellationToken: cancellationToken));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (mailboxId.HasValue)
        {
            ...
            var rows = await connection.QueryAsync<MailboxMessageRow>(new CommandDefinition(
                """
                SELECT
        ...
        var inboxCount = await connection.QuerySingleAsync<int>(new CommandDefinition(
            """
            SELECT COUNT(*)
            FROM MailboxMessages AS mm
            INNER JOIN Mailboxes AS mb ON mb.Id = mm.MailboxId
        ...
        var inboxRows = await connection.QueryAsync<MailboxMessageRow>(new CommandDefinition(
            """
            SELECT
                mm.Id AS MailboxMessageId,
                mm.MailboxId AS MailboxMessageMailboxId,
    ...
    {
        await using var connection = await this.OpenConnectionAsync(cancellationToken);
        var row = await connection.QuerySingleOrDefaultAsync<MailboxMessageRow>(new CommandDefinition(
            """
            SELECT
                mm.Id AS MailboxMessageId,
                mm.MailboxId AS MailboxMessageMailboxId,
    ...
    private sealed class MailboxStatsRow
    {
        public int Id { get; set; }
    ...
    private sealed class MailboxMessageRow
    {
